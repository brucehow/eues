...
def gui():
	window = tk.Tk()
	window.title('EUES')
	ft1 = tkFont.Font(family='Fixdsys', size=25, weight=tkFont.BOLD)
	...
	def button1_func(infile, th_begin, th_end, wavelet, sample_interval, testcase_name):
		...
		err, ret = kernels.start_analysis(infile, th_begin, th_end, wavelet, sample_interval, testcase_name)
		if True != err:
		...
		s13.set(ret[12])
		s14.set(ret[13])
		#window.geometry('{}x{}'.format(2*W, 2*H))
		'''
		pil_image11 = Image.open(r'./output/{}_1.png'.format(testcase_name)) 
		...
		pil_image21 = Image.open(r'./output/{}_2.png'.format(testcase_name)) 
	...
	def button2_func(testcase_name):
		...
		print("button2_func down\n")



	#frame = tk.Frame(window, bd=1)
	#frame.pack(side='left')
	#frame1 = tk.Frame(frame, bd=1)
	...
	tk.Label(frame12, text="time start(hour)", font=ft1).grid(row=1,sticky=tk.W)
	tk.Label(frame12, text="time end(hour)", font=ft1).grid(row=2,sticky=tk.W)
	tk.Label(frame12, text="wavelet", font=ft1).grid(row=3,sticky=tk.W)
	tk.Label(frame12, text="sample interval(s)", font=ft1).grid(row=4, sticky=tk.W)
	tk.Label(frame12, text="test case name", font=ft1).grid(row=5, sticky=tk.W)
	tk.Button(frame12, text = "select path", font=ft1, command = selectPath).grid(row = 0, column = 2, sticky=tk.W)
	e1 = tk.Entry(frame12,textvariable = path,font=ft1)
	e2 = tk.Entry(frame12, font=ft1)
	e3 = tk.Entry(frame12, font=ft1)
	e4 = ttk.Combobox(frame12, font=ft1, values=['haar','db1', 'db2', 'db3', 'db4', 'db5', 'db6', 'db7',\
	...
	button1 = tk.Button(frame12, text='start', font=ft1, command=lambda: button1_func(e1.get(), \
			e2.get(), e3.get(), e4.get(), e5.get(), e6.get())).grid(row=6, column=1, sticky=tk.W)
	
	#window.geometry('{}x{}'.format(W, H))

	tk.Label(frame13, font=ft1, text="EUEs number").grid(row=0, sticky=tk.W)
	tk.Label(frame13, font=ft1,text="wavelets number").grid(row=1,sticky=tk.W)
	tk.Label(frame13, font=ft1,text="features").grid(row=2,sticky=tk.W)
	tk.Label(frame13, font=ft1,text="MEAN").grid(row=2, column=1,sticky=tk.W)
	tk.Label(frame13, font=ft1,text="SD").grid(row=2, column=2, sticky=tk.W)
	tk.Label(frame13, font=ft1,text="SEM").grid(row=2, column=3, sticky=tk.W)

	tk.Label(frame13, font=ft1,text="amplitude").grid(row=3, sticky=tk.W)
	tk.Label(frame13, font=ft1,text="duration").grid(row=4, sticky=tk.W)
	tk.Label(frame13, font=ft1,text="area").grid(row=5, sticky=tk.W)
	tk.Label(frame13, font=ft1,text="inter time").grid(row=6, sticky=tk.W)
	button2 = tk.Button(frame13, font=ft1,text='dump result', command=lambda: button2_func(e6.get())).grid(row=7, column=2, sticky=tk.W)
	#tk.Button(frame13, text = "select path", command = selectPath).grid(row = 7, column = 2, sticky=tk.W)
	#e7 = tk.Entry(frame13,textvariable = path)
