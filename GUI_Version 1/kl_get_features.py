...
def get_features(sig, fs = 1/60):
	...
	#pdb.set_trace()
	onsets = remove_fakeonsets(sig, onsets, 0.25*np.std(baseline))
...
def dump_features(eues_info, filename, pos):
	t_onsets, t_peaks, t_tobaseline, t_duration, amplitude, areas, t_inter = eues_info
	t_onsets, t_peaks, t_tobaseline, t_duration, amplitude, areas, t_inter = \
	t_onsets[pos], t_peaks[pos], t_tobaseline[pos], t_duration[pos], amplitude[pos], \
	areas[pos], t_inter[pos]
